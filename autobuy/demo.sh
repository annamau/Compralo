...
# E and F are amazon listings. The simulator holds them so the agent can see and evaluate them,
# but their /checkout never reaches the simulator: P4 routes any non-store-* retailer to Zinc,
# which places the order with its own account. The url is the Zinc sandbox rehearsal slug.
OFFER_E='{"id":"offer-e","retailer":"amazon","listing_title":"Sony PlayStation 5 Slim Digital Edition 1TB Console","price":447,"shipping":0,"currency":"EUR","condition":"new","in_stock":true,"url":"https://zinc.com/shop/products/test-success"}'
OFFER_F='{"id":"offer-f","retailer":"amazon","listing_title":"PlayStation 5 Slim Digital Edition 1TB","price":446,"shipping":0,"currency":"EUR","condition":"new","in_stock":true,"url":"https://zinc.com/shop/products/test-out-of-stock"}'
...
case "${1:-run}" in
  ...
  order)
    ...
    BODY+='"constraints":{"max_total":450,"currency":"EUR","quantity":1,"condition":"new","approved_retailers":["store-a","store-b","store-c","amazon"],"deadline":"'"$DEADLINE"'","variant":{"edition":"digital","storage":"1TB"},"allow_bundles":false}'
  ...
  --zinc) ORDER=(A B E) ;;
  run) ORDER=(A B C D) ;;
  *) echo "usage: ./demo.sh [reset | order [3ds] | run | --mismatch-first | --zinc]"; exit 1 ;;
esac
...
for step in "${ORDER[@]}"; do
  case $step in
    ...
    E) label="OFFER E  amazon   'Sony PlayStation 5 Slim Digital Edition 1TB Console'  EUR 447 delivered"
       expect="expect QUALIFIED → LOCK → REVALIDATED → CHECKOUT_OK → PAYMENT_CAPTURED → PURCHASED via Zinc (order ref W…), not the simulator"; body=$OFFER_E ;;
    F) label="OFFER F  amazon   'PlayStation 5 Slim Digital Edition 1TB'  EUR 446 delivered"
       expect="expect QUALIFIED → LOCK → REVALIDATED → Zinc says out of stock → FAILED, hold refunded, NEEDS_ATTENTION (re-arm)"; body=$OFFER_F ;;
